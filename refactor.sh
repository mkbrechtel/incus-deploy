...
# fail if there is any file staged for commit
if ! git diff --cached --quiet; then
  echo "Assertion failed: Files are staged for commit"
  exit 1
fi
...
git add refactor.sh
git commit --signoff -m "Hacky refactoring script that is supposed to do the refactoring for me. "
...
refactoring_tag="refactoring-$(date -u +"%Y%m%dT%H%M%S")"
git tag "$refactoring_tag" HEAD

git push origin refactoring $refactoring_tag

git reset --hard main
git checkout refactoring ansible/
git checkout $refactoring_tag refactor.sh
