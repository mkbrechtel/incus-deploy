...
for book in "${books[@]}"; do
    mkdir -p "ansible/books/roles/${book}/tasks" "ansible/books/roles/${book}/vars" "ansible/books/roles/${book}/files" "ansible/books/roles/${book}/templates" "ansible/books/roles/${book}/defaults" "ansible/books/roles/${book}/meta"
done
...
for book in "${books[@]}"; do
    ...
    # reset defaults file
    echo '---' > "ansible/books/roles/${book}/defaults/main.yaml"
    ...
    for var in $default_vars; do
        ...
        echo $var: "$default_val" >> "ansible/books/roles/${book}/defaults/main.yaml"
...
for book in "${books[@]}"; do
    ...
    # reset vars file
    echo '---' > "ansible/books/roles/${book}/vars/main.yaml"
    ...
    for var in $non_default_vars; do
        ...
        if [ ! -z "$val" ]; then
            echo "$val" >> "ansible/books/roles/${book}/vars/main.yaml"
        fi
    done
done
...
for book in "${books[@]}"; do
    ...
    mkdir -p "ansible/books/roles/${book}/templates"
    # Move all files from the book's files directory to templates
    if [ -d "ansible/files/${book}" ] && [ "$(ls -A "ansible/files/${book}")" ]; then
        mv "ansible/files/${book}"/* "ansible/books/roles/${book}/templates/"
    fi
done

# remove the ../files/${book}/ mentions in the files
for book in "${books[@]}"; do
    if [ -d "ansible/books/roles/${book}/templates" ] && [ "$(ls -A "ansible/books/roles/${book}/templates")" ]; then
    for file in "ansible/books/${book}.yaml" "ansible/books/roles/${book}/templates"/* "ansible/books/roles/${book}/vars/main.yaml"; do
...
for book in "${books[@]}"; do
    # Add changed files
    git add "ansible/books/${book}.yaml" "ansible/files/${book}/" "ansible/books/roles/${book}/"
